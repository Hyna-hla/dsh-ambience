window.__ModuleLoader__.load({ id: 'dsh-ambience', factory: (require) => { var module = { exports: {} }; var exports = module.exports;
"use strict";
...
window.__ModuleLoader__.load({
  ...
  factory: function(require2) {
    ...
    var EQ_BANDS = [31, 62, 125, 250, 500, 1e3, 2e3, 4e3, 8e3, 16e3];
    var EQ_PRESETS = {
    ...
    function loadSettings() {
      try {
        ...
        if (Array.isArray(raw.tracks)) {
          d.tracks = raw.tracks.filter(function(t) {
            return t && typeof t.name === "string" && typeof t.url === "string" && t.url.indexOf("data:") === 0;
          });
        }
        return d;
      } catch (e) {
    ...
    function saveSettings(s) {
      try {
        ...
        copy.tracks = (copy.tracks || []).filter(function(t) {
          return t && t.url && t.url.indexOf("data:") === 0 && t.url.length <= AUDIO_PERSIST_MAX;
        });
    ...
    function notify() {
      runtime.listeners.forEach(function(fn) {
        try {
          fn();
        } catch (e) {
        }
    ...
    function togglePlay() {
      if (runtime.playing) {
        ...
        if (runtime.currentIdx < 0 && allTracks().length) {
          playTrack(0);
          return;
        }
    ...
    function nextTrack() {
      ...
      if (repeat === "one" && runtime.currentIdx >= 0) {
        playTrack(runtime.currentIdx);
        return;
      }
      var idx;
    ...
    function injectStyles() {
      ...
      styleEl.textContent = ".amb-root{position:fixed;right:14px;bottom:14px;z-index:2147483000;display:flex;flex-direction:column;align-items:flex-end;gap:8px;pointer-events:none;font-family:inherit}.amb-pill{pointer-events:auto;display:flex;align-items:center;gap:6px;padding:6px 10px;border-radius:999px;background:var(--dsw-alias-bg-overlay,#222);border:1px solid var(--dsw-alias-border-l1,#444);box-shadow:0 4px 16px rgba(0,0,0,.25);color:var(--dsw-alias-label-primary,#eee);backdrop-filter:blur(10px);font-size:12px}.amb-btn{pointer-events:auto;border:1px solid var(--dsw-alias-border-l1,#444);background:transparent;color:var(--dsw-alias-label-primary,#eee);border-radius:6px;padding:2px 8px;font-size:12px;cursor:pointer;line-height:1.6}.amb-btn:hover{border-color:var(--dsw-alias-brand-primary,#4d7cfe)}.amb-panel{pointer-events:auto;width:330px;max-height:72vh;overflow:auto;border-radius:12px;background:var(--dsw-alias-bg-overlay,#1d1d1f);border:1px solid var(--dsw-alias-border-l1,#444);box-shadow:0 8px 32px rgba(0,0,0,.35);color:var(--dsw-alias-label-primary,#eee);backdrop-filter:blur(14px);font-size:12px}.amb-tabs{display:flex;gap:4px;padding:8px 8px 0}.amb-tab{padding:4px 10px;border-radius:6px;cursor:pointer;border:1px solid transparent;color:var(--dsw-alias-label-secondary,#bbb)}.amb-tab.active{border-color:var(--dsw-alias-border-l2,#555);background:var(--dsw-alias-bg-layer-1,#2a2a2c);color:var(--dsw-alias-label-primary,#eee)}.amb-body{padding:4px 12px 12px}.amb-row{display:flex;align-items:center;gap:6px;margin:6px 0}.amb-track{display:flex;justify-content:space-between;gap:6px;padding:4px 6px;border-radius:6px;cursor:pointer}.amb-track:hover{background:var(--dsw-alias-bg-layer-1,#2a2a2c)}.amb-track.playing{color:var(--dsw-alias-brand-primary,#4d7cfe)}.amb-title{font-size:11px;color:var(--dsw-alias-label-secondary,#999);margin:2px 0}.amb-track-name{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:220px}input[type=range].amb-range{flex:1;accent-color:var(--dsw-alias-brand-primary,#4d7cfe)}@keyframes amb-breathe{0%{opacity:var(--amb-lo,.4)}100%{opacity:var(--amb-hi,.8)}}";
    ...
    function Slider(props) {
      return h(
        ...
        h(
          "span",
          { style: { fontSize: 11, color: "var(--dsw-alias-label-secondary,#999)" } },
    ...
    function BgTab(props) {
      ...
      return h(
        ...
        h(
          ...
          h(
            "label",
            { className: "amb-btn", style: { display: "inline-flex", alignItems: "center", gap: 4, cursor: "pointer" } },
            h("input", { type: "checkbox", checked: b.on, onChange: function(ev) {
              set("on", ev.target.checked);
            } }),
            " \u542F\u7528\u80CC\u666F"
          ),
          h("span", { style: { flex: 1 } }),
          Btn({ label: "\u{1F4C1} \u5BFC\u5165\u56FE\u7247", onClick: function() {
            props.openImgPicker();
          } })
        ),
        b.image ? Btn({ label: "\u2715 \u79FB\u9664\u56FE\u7247", onClick: function() {
          set("image", null);
        } }) : null,
        h("div", { className: "amb-title" }, "\u652F\u6301 JPG / PNG / WebP\uFF1B\u22642.5MB \u7684\u56FE\u7247\u4F1A\u6301\u4E45\u4FDD\u5B58\uFF0C\u91CD\u542F\u540E\u4ECD\u5728"),
        Slider({ label: "\u56FE\u7247\u900F\u660E\u5EA6", value: Math.round(b.opacity * 100), min: 5, max: 100, suffix: "%", onChange: function(v) {
          set("opacity", v / 100);
        } }),
        Slider({ label: "\u6A21\u7CCA", value: b.blur, min: 0, max: 30, suffix: "px", onChange: function(v) {
          set("blur", v);
        } }),
        h(
          "div",
          ...
          h(
            ...
            h("input", { type: "checkbox", checked: b.autoFade, onChange: function(ev) {
              set("autoFade", ev.target.checked);
            } }),
            " \u81EA\u4E3B\u547C\u5438\u6DE1\u5316\uFF08\u7F13\u6162\u660E\u6697\u5FAA\u73AF\uFF09"
          )
        ),
    ...
    function MusicTab(props) {
      ...
      return h(
        ...
        h(
          "div",
          { className: "amb-row" },
          Btn({ label: "\u23EE", title: "\u4E0A\u4E00\u66F2", onClick: function() {
            prevTrack();
          } }),
          Btn({ label: runtime.playing ? "\u23F8" : "\u25B6", title: "\u64AD\u653E/\u6682\u505C", onClick: function() {
            togglePlay();
          } }),
          Btn({ label: "\u23ED", title: "\u4E0B\u4E00\u66F2", onClick: function() {
            nextTrack();
          } }),
          h("span", { style: { flex: 1 } }),
          Btn({ label: "\u{1F4C1} \u5BFC\u5165\u97F3\u4E50", onClick: function() {
            props.openAudioPicker();
          } })
        ),
        h(
          "div",
          ...
          Select({ value: m.repeat, onChange: function(v) {
            set("repeat", v);
          }, options: [
            { value: "off", label: "\u5173\u95ED" },
          ...
          h(
            ...
            h("input", { type: "checkbox", checked: m.shuffle, onChange: function(ev) {
              set("shuffle", ev.target.checked);
            } }),
            "\u968F\u673A"
          )
        ),
        Slider({ label: "\u97F3\u91CF", value: m.volume, min: 0, max: 200, suffix: "%", onChange: function(v) {
          set("volume", v);
        } }),
        h("div", { className: "amb-title" }, "\u64AD\u653E\u5217\u8868\uFF08\u70B9\u51FB\u64AD\u653E\uFF1B\u5927\u6587\u4EF6\u4EC5\u672C\u4F1A\u8BDD\u6709\u6548\uFF09\uFF1A" + tracks.length + " \u9996"),
        tracks.length === 0 ? h("div", { className: "amb-title" }, "\u5C1A\u672A\u5BFC\u5165\u97F3\u4E50\u3002\u652F\u6301 FLAC / MP3 / WAV / M4A / OGG") : tracks.map(function(t, i) {
          return h(
            "div",
            {
              className: "amb-track" + (i === runtime.currentIdx ? " playing" : ""),
              key: i,
              onClick: function() {
                playTrack(i);
              }
            ...
            Btn({ label: "\u2715", title: "\u79FB\u9664", onClick: function(ev) {
              ev.stopPropagation();
              props.removeTrack(i);
            } })
          );
      ...
      return h(
        ...
        h(
          ...
          h(
            "label",
            { style: { display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12 } },
            h("input", { type: "checkbox", checked: m.eqOn, onChange: function(ev) {
              set("eqOn", ev.target.checked);
            } }),
            " \u5747\u8861\u5668\u542F\u7528\uFF08\u53D6\u6D88\u52FE\u9009=\u65C1\u8DEF\uFF09"
          ),
          h("span", { style: { flex: 1 } }),
          Select({ value: m.preset, onChange: function(v) {
            props.applyPreset(v);
          }, options: Object.keys(EQ_PRESETS).map(function(k) {
            return { value: k, label: k };
          }) })
        ),
        Slider({ label: "\u9884\u653E\u5927", value: m.preamp, min: -12, max: 12, step: 0.5, suffix: "dB", onChange: function(v) {
          set("preamp", v);
        } }),
        EQ_BANDS.map(function(freq, i) {
          return h(
            "div",
            { key: freq, style: { display: "flex", alignItems: "center", gap: 6, margin: "4px 0" } },
            h("span", { style: { width: 44, fontSize: 11, color: "var(--dsw-alias-label-secondary,#999)", textAlign: "right" } }, freq >= 1e3 ? freq / 1e3 + "k" : String(freq)),
            h("input", {
              type: "range",
              className: "amb-range",
              min: -12,
              ...
              value: m.gains[i] || 0,
              style: { flex: 1 },
              onChange: /* @__PURE__ */ (function(idx) {
                return function(ev) {
                  var g = m.gains.slice();
    ...
    function App() {
      ...
      useEffect(function() {
        ...
        var unsub = function() {
          runtime.listeners = runtime.listeners.filter(function(f) {
            return f !== unsub;
          });
        };
      ...
      function removeTrack(i) {
        ...
        if (i < persistedCount) {
          setSettings(function(prev) {
            var next = JSON.parse(JSON.stringify(prev));
            next.tracks = next.tracks.filter(function(x, idx) {
              return idx !== i;
            });
            return next;
          });
          ...
          runtime.sessionTracks = runtime.sessionTracks.filter(function(x, idx) {
            return idx !== si;
          });
          notify();
        }
          ...
          reader.onload = function() {
            ...
            setSettings(function(prev) {
              ...
              if (dataUrl.length <= AUDIO_PERSIST_MAX) {
                ...
                runtime.sessionTracks.push({ name: file.name, url });
              }
              return next;
            });
          };
      ...
      return h(
        ...
        h("input", {
          ...
          style: { display: "none" },
          multiple: true,
          accept: ".flac,.mp3,.wav,.m4a,.ogg,audio/flac,audio/mpeg,audio/wav,audio/x-flac,audio/ogg",
        ...
        open ? h(
          ...
          h(
            ...
            ["music", "bg", "eq"].map(function(t) {
              var label = t === "music" ? "\u{1F3B5} \u97F3\u4E50" : t === "bg" ? "\u{1F5BC} \u80CC\u666F" : "\u{1F39A} \u8C03\u97F3\u53F0";
              return h("div", {
                className: "amb-tab" + (tab === t ? " active" : ""),
                key: t,
                onClick: function() {
                  setTab(t);
                }
            ...
            h("span", { style: { flex: 1 } }),
            h("div", { className: "amb-tab", onClick: function() {
              setOpen(false);
            } }, "\u2715")
          ),
          h(
            ...
            tab === "eq" ? h(EqTab, { settings, patchMusic, applyPreset }) : null
          )
        ) : null,
        h(
          "div",
          { className: "amb-pill" },
          h("button", { className: "amb-btn", title: "\u4E0A\u4E00\u66F2", onClick: function() {
            prevTrack();
          } }, "\u23EE"),
          h("button", { className: "amb-btn", title: "\u64AD\u653E/\u6682\u505C", onClick: function() {
            togglePlay();
          } }, playing ? "\u23F8" : "\u25B6"),
          h("button", { className: "amb-btn", title: "\u4E0B\u4E00\u66F2", onClick: function() {
            nextTrack();
          } }, "\u23ED"),
          h(
            "span",
          ...
          h("button", { className: "amb-btn", title: "\u5C55\u5F00\u9762\u677F", onClick: function() {
            setOpen(!open);
          } }, "\u{1F39B}")
        )
      );
    ...
    function SettingsSection() {
      ...
      useEffect(function() {
        ...
        runtime.listeners.push(function() {
          setSettings(JSON.parse(JSON.stringify(runtime.settings)));
        });
        return unsub;
      }, []);
      useEffect(function() {
      ...
      return h(
        ...
        h("p", null, "\u5BFC\u5165\u80CC\u666F\u56FE\u540E\u53EF\u8C03\u900F\u660E\u5EA6/\u6A21\u7CCA\uFF1B\u97F3\u4E50\u4E0E\u5747\u8861\u5668\u7684\u5B8C\u6574\u9762\u677F\u5728\u9875\u9762\u53F3\u4E0B\u89D2\u60AC\u6D6E\u6761\uFF08\u{1F39B}\uFF09\u3002"),
        h(
          "div",
          { className: "amb-row" },
          h(
        ...
        Slider({ label: "\u56FE\u7247\u900F\u660E\u5EA6", value: Math.round(b.opacity * 100), min: 5, max: 100, suffix: "%", onChange: function(v) {
          patchBg("opacity", v / 100);
        } }),
        Slider({ label: "\u6A21\u7CCA", value: b.blur, min: 0, max: 30, suffix: "px", onChange: function(v) {
          patchBg("blur", v);
        } }),
        Slider({ label: "\u97F3\u4E50\u97F3\u91CF", value: m.volume, min: 0, max: 200, suffix: "%", onChange: function(v) {
          patchMusic("volume", v);
        } })
      );
    }
      ...
      try {
        ...
        if (ctx.slots && typeof ctx.slots.inject === "function") {
          try {
            ...
            console.error("[dsh-ambience] shell.overlay \u6CE8\u518C\u5931\u8D25", e);
          }
          try {
            ctx.slots.inject("settings.section", function() {
              return ctx.slots.register(
                { name: "settings.section", id: "ambience", order: 55, label: function() {
                  return "Ambience \u80CC\u666F\u4E0E\u97F3\u4E50";
                } },
                function() {
                  return h(SettingsSection, null);
                }
              );
            });
            console.info("[dsh-ambience] settings.section \u5DF2\u6CE8\u518C");
          } catch (e) {
            console.error("[dsh-ambience] settings.section \u6CE8\u518C\u5931\u8D25", e);
          }
        } else {
